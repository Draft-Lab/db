...
export class CoreSQLiteMemory implements SQLiteBackend {
	...
	async init(): Promise<void> {
		...
		this.db.exec({ sql: "PRAGMA cache_size = -32000" })
	...
	async exec(statement: DriverStatement): Promise<RawResultData> {
		...
		if (!this.db) {
			throw new Error("@draftlab/db: Database not initialized")
		}
	...
	async importDatabase(data: ArrayBuffer): Promise<void> {
		...
		if (!this.db.pointer) {
			throw new Error("@draftlab/db: Failed to get database pointer")
		}
